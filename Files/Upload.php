...
define('TEMP_REAL_DIR', ROOTDIR . '/' . TEMP_DIR);
...
class Upload
{
    ...
    private function check_tmpfile($userfile, $no_check_size)
    {
        ...
        if (! empty($this->config['maxsize']) and $userfile['size'] > $this->config['maxsize']) {
            if (! ($no_check_size and preg_match('#image\/[x\-]*([a-z]+)#', $this->file_mime))) {
                return sprintf(_ERROR_UPLOAD_MAX_USER_SIZE, e_convertfromBytes($this->config['maxsize']));
            }
        }
    ...
    {
        if (defined('LANG_DATA') and file_exists(ROOTDIR . '/includes/utf8/lookup_' . LANG_DATA . '.php')) {
            include ROOTDIR . '/includes/utf8/lookup_' . LANG_DATA . '.php' ;
            $word = strtr($word, $utf8_lookup_lang);
        }

        if (file_exists(ROOTDIR . '/includes/utf8/lookup.php')) {
            $utf8_lookup = false;
            include ROOTDIR . '/includes/utf8/lookup.php' ;
    ...
    public function save_urlfile($urlfile, $savepath, $replace_if_exists = true, $no_check_size = false)
    {
        ...
        $this->temp_file = str_replace("\\", "/", tempnam(TEMP_REAL_DIR, TEMPNAM_PREFIX));
