...
class FloodBlocker
{
    ...
    {
        $info = array();
        $logfile = $this->logs_path . $this->ip_addr . '.' . LOGS_EXT;
        ...
        foreach ($this->rules as $interval => $limit) {
            if (! isset($info[$interval])) {
                $info[$interval]['time'] = CURRENTTIME;
                $info[$interval]['count'] = 0;
            }

            ++$info[$interval]['count'];

            if (CURRENTTIME - $info[$interval]['time'] > $interval) {
                $info[$interval]['count'] = 1;
                $info[$interval]['time'] = CURRENTTIME;
            }

            if ($info[$interval]['count'] > $limit) {
                $this->time_blocker = 1 + (CURRENTTIME - $info[$interval]['time'] - $interval) * - 1;
