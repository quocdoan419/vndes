...
class Error
{
    ...
    public function __construct($config)
    {
        ...
        $this->day = date('d-m-Y', CURRENTTIME);
        $this->error_date = date('r', CURRENTTIME);
        $this->month = date('m-Y', CURRENTTIME);
    ...
    private function get_error_log_path($path)
    {
        ...
        if (is_dir(ROOTDIR . '/' . $path)) {
            ...
            foreach ($e as $p) {
                ...
                if (! is_dir(ROOTDIR . '/' . $cp . $p)) {
                    if (! @mkdir(ROOTDIR . '/' . $cp . $p, 0777)) {
                        $cp = '';
                        break;
                    }
                }
    ...
    private function info_die()
    {
        ...
        $size = @getimagesize(ROOTDIR . '/' . $this->site_logo);
    ...
    public function error_handler($errno, $errstr, $errfile, $errline)
    {
        ...
        if (isset($errfile) and ! empty($errfile)) {
            $this->errfile = str_replace(ROOTDIR, '', str_replace('\\', '/', $errfile));
        }
