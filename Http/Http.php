...
class Http
{
    ...
    private function request($url, $args)
    {
        $defaults = array(
            ...
            'httpversion' => 1.0,
            'user-agent' => 'Vndes CMS ' . Http::$site_config['version'] . '. Developed by VINADES. Url: http://Vndes.vn. Code: ' . md5(Http::$site_config['sitekey']),
            'referer' => null,
        ...
        // Determine if this request is to OUR install of Vndes
    ...
    {
        if (!empty($args['cookies'])) {
            // Upgrade any name => value cookie pairs to Vndes\Http\Cookie instances
